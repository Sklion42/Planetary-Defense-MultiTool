...
async function fetchRow(requestId) {
  ...
  if (!row) return null;
  // Certains nœuds ignorent upper_bound et renvoient la ligne suivante : sans
  // cette vérification on révélerait la requête d'un autre joueur.
  return String(row.request_id) === id ? row : null;
}
...
// Le front appelle cette route dans la foulée du commit : la ligne a alors moins
// de MIN_AGE_SEC. On attend l'âge minimum au lieu d'abandonner, sinon ce premier
// appel ne sert à rien et le reveal n'a lieu qu'au retrigger suivant.
export async function revealOne(requestId) {
  ...
  for (let attempt = 0; attempt < 4; attempt += 1) {
    const age = nowSec() - Number(row.commit_time || 0);
    if (age >= MIN_AGE_SEC) break;
    await sleep(Math.max(250, (MIN_AGE_SEC - age + 0.25) * 1000));
    row = await fetchRow(id);
    if (!row) return false; // résolue entre-temps
  }
  if (nowSec() - Number(row.commit_time || 0) < MIN_AGE_SEC) return false;
